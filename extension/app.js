...
function buildOverflowChips(hiddenTabs, urlCounts = {}) {
  const hiddenChips = hiddenTabs.map(tab => {
    ...
    return `<div class="page-chip clickable${chipClass}" data-action="focus-tab" data-tab-url="${safeUrl}" title="${safeTitle}">
      ${faviconUrl ? `<img class="chip-favicon" src="${faviconUrl}" alt="" data-hide-on-error="1">` : ''}
    ...
    <div class="deferred-item" data-deferred-id="${item.id}">
      ...
      <div class="deferred-info">
        <a href="${item.url}" target="_blank" rel="noopener" class="deferred-title" title="${(item.title || '').replace(/"/g, '&quot;')}">
          <img src="${faviconUrl}" alt="" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px" data-hide-on-error="1">${item.title || item.url}
        </a>
...
/* ----------------------------------------------------------------
   FAVICON FALLBACKS — hide <img data-hide-on-error="1"> on load failure.
   Listens at the document level (capture phase, since `error` doesn't
   bubble) to replace the inline `onerror=` handlers we removed for CSP.
   ---------------------------------------------------------------- */
document.addEventListener('error', (e) => {
  const t = e.target;
  if (t && t.tagName === 'IMG' && t.dataset.hideOnError) {
...
(function setupGoogleAppsLauncher() {
  ...
  function positionMenu() {
    // Park the menu at the viewport's right edge (24px from edge),
    // top-aligned with the waffle button. Falls back to under-button
    // dropdown when the right-side empty space is too narrow.
    const r = btn.getBoundingClientRect();
    const MENU_WIDTH = 280;     // ~3 tiles * 80px + padding/gaps
    const RIGHT_GAP = 24;
    const spaceOnRight = window.innerWidth - r.right;
    if (spaceOnRight >= MENU_WIDTH + RIGHT_GAP) {
      // Wide viewport: park in the right margin, beside the button.
      menu.style.top = `${Math.round(r.top)}px`;
      menu.style.right = `${RIGHT_GAP}px`;
    } else {
      // Narrow viewport: drop down under the button instead.
